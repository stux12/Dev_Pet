...
1. `DevPet_0.2.14_x64_en-US.msi` 를 실행해 설치 (또는 릴리스에서 다운로드)
2. 시작 메뉴에서 **DevPet** 실행
...
| 설치 파일(MSI) | `src-tauri/target/release/bundle/msi/DevPet_0.2.14_x64_en-US.msi` | 정식 설치 / **다른 PC 배포** |

- 예시 전체 경로: `C:\...\Dev_Pet\src-tauri\target\release\bundle\msi\DevPet_0.2.14_x64_en-US.msi`
...
### 2026-07-18 · v0.2.14
- **자동 업데이트 동작 확인용 릴리스** — 기능 변화는 없습니다. v0.2.13의 자동 업데이트(⬆️ 버튼)가 실제로 동작하는지 확인하기 위한 테스트 버전입니다.
...
### 2026-07-16 · v0.2.9
- **디스코드 승인 알림에 작업 요약 표시** — 승인 프롬프트 상단에 뜨는 문구(도구의 `description`, 예: `Create v0.2.8 release, upload MSI, cleanup`)를 detail로 우선 표시합니다. 원시 명령보다 읽기 좋습니다. `description`이 없으면 실제 명령/경로로 폴백합니다. (데스크탑 세션 한정 — CLI 승인은 훅 구조상 명령/요약을 알 수 없어 `확인이 필요해요`로 표시됩니다)

### 2026-07-16 · v0.2.8
- **디스코드 승인 알림에 명령 표시** — 디스코드 승인 알림에 **어떤 도구/명령을 쓰려는지**를 함께 보여줍니다. 예: `Bash: cd /c/… && ls`, `Write: C:\Users\…\test.txt`. 완료 알림은 그대로 상세 없이 제목 + 시간·토큰만. (데스크탑 세션에서 감지된 명령이며, CLI 승인은 명령을 알 수 없어 `확인이 필요해요`로 표시됩니다. 펫 말풍선·리스트는 계속 간결하게 제목만)
